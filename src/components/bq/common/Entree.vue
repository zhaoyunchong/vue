<template>
    <div>
        <div class="card" v-for="(item,i) of entree" :key="i"> 
                    <!-- 左边图片 -->
                    <div class="card-img">
                    ...
                    <div class="cart-add">
                        <!-- 减号 -->
                        <div class="cart-reduce" @click="sub"><img src="../../../assets/jian.png" :data-i="i"></div>
                        <span class="cart-inner">0</span>
                        <!-- 加号 -->
                        <div class="cart-push" @click="add"><img src="../../../assets/jia.png" :data-i="i"></div>
                    </div>
                </div>     
            </div>          
</template>
...
export default {
    ...
    add(event){
          ...
          this.entree.push({"spname":this.list[i].spname,"price":this.list[i].price,"num":event.target.parentNode.previousElementSibling.innerHTML});
          var n = this.entree.length;
          console.log(n)
        //   console.log(this.all[0].spname)
          for(var a=0;a<n-1;a++){
            if(n>1){
                if(this.entree[n-1].spname==this.entree[a].spname){
                    if((n-1)>a){
                    this.entree.splice(a,1)
                    a+=2
            this.$store.commit('add',this.entree);
            this.$store.commit('sub',this.entree);
