    ...
    <div>
        <!-- 商品卡片 -->               
           <div class="card" v-for="(item,i) of list" :key="i">         
            ...
            <div class="cart-add">
               <!-- 减号 -->    
                <div class="cart-reduce" @click="sub1"><img src="../../../assets/jian.png"  :data-i="i"></div>
                <span class="cart-inner">0</span>
                <!-- 加号 -->
                <div class="cart-push" @click="add1"><img src="../../../assets/jia.png"  :data-i="i"></div>
            </div>
        </div>
    </div>
</template>
     ...
     methods:{
         add1(event){
          ...
          if(num>0){
            ...
            //   event.currentTarget.previousElementSibling.previousElementSibling.style.transition="1s";
            //   event.currentTarget.previousElementSibling.previousElementSibling.style.transform="rotateZ(360deg)";

          }
          this.all.push({"spname":this.list[i].spname,"price":this.list[i].price,"num":event.target.parentNode.previousElementSibling.innerHTML});
          var n = this.all.length;
          console.log(n)
        //   console.log(this.all[0].spname)
          for(var a=0;a<n-1;a++){
            if(n>1){
                if(this.all[n-1].spname==this.all[a].spname){
                    if((n-1)>a){
                    this.all.splice(a,1)
                    a+=2
            this.$store.commit('add',this.all);
            this.$store.commit('sub',this.all);
