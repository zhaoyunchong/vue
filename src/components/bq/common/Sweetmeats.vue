<template>
    <div>
        <!--甜品 锚点-->
                <div class="card" v-for="(item,i) of sweetmeats" :key="i">         
                    <!-- 左边图片 -->
                    <div class="card-img">
                    ...
                    <div class="cart-add">
                        <!-- 减号 -->
                        <div class="cart-reduce" @click="sub4"><img src="../../../assets/jian.png" :data-i="i"></div>
                        <span class="cart-inner">0</span>
                        <!-- 加号 -->
                        <div class="cart-push" @click="add4"><img src="../../../assets/jia.png" :data-i="i"></div>
                    </div>
                </div>
    </div>
</template>
    ...
    created(){
          ...
          this.axios.get(url,{params:obj}).then(res=>{
              ...
              for(var i=0;i<f.length;i++){
                  if(f[i].typeid==4){
                      this.sweetmeats.push(f[i]);
                  }
              }
            // this.list = res.data.data
              console.log(this.sweetmeats)
          })
    },
    methods:{
        add4(event){
          ...
          this.sweetmeats.push({"spname":this.list[i].spname,"price":this.list[i].price,"num":event.target.parentNode.previousElementSibling.innerHTML});
          var n = this.sweetmeats.length;
          console.log(n)
        //   console.log(this.all[0].spname)
          for(var a=0;a<n-1;a++){
            if(n>1){
                if(this.sweetmeats[n-1].spname==this.sweetmeats[a].spname){
                    if((n-1)>a){
                    this.sweetmeats.splice(a,1)
                    a+=2
            this.$store.commit('add',this.sweetmeats);
            this.$store.commit('sub',this.sweetmeats);
